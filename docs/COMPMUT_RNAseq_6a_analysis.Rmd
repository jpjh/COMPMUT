...
How many additional genes are differentially expressed with ∆gacS, beyond the 2x DE genes plotted above?
...
Examine all genes carried by Tn6291. Note that PFLU_1904 is called PFLU_1903A, and there is no PFLU_1889.
...
No evidence of significant upregulation of any other Tn6291 genes following plasmid acquisition (one downregulated with pQBR57).

```{r}
full_table_chr[tn6291,grep("pQBR[0-9]*\\.dPFLU4242.(logFC|FDR)", colnames(full_table_chr))] %>% kable()
```

No evidence of significant upregulation of any other Tn6291 genes following chromosomal amelioration. 

```{r}
full_table_chr[tn6291,grep("pQBR[0-9]*\\.dgacS.(logFC|FDR)", colnames(full_table_chr))] %>% kable()
```

No evidence of significant upregulation of any other Tn6291 genes following *gacS* amelioration (PFLU_1898 is down for both plasmids). 
...
Conclusion: only the putative transposase-encoding genes are upregulated, suggesting that it is specific upregulation rather than gene duplication causing this pattern. 
